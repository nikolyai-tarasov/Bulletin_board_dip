...
class AdTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create(
            email='kolya.tarasov1@mail.com',
            username='kuzon12', password='pass1'
        )
        self.ad = BulletinBoard.objects.create(
            title="RX 590 8GB", price=7890,
            description="Good", author=self.user
        )
    ...
    def test_create_ad(self):
        ...
        self.assertEqual(
            BulletinBoard.objects.all().count(),
            2
        )
...
class ReviewTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create(
            ...
            username='kuzon2', password='pass2'
        )

        self.user_1 = User.objects.create(
            email='kolya.tarasov222@mail.com',
            username='kuzon222', password='pass234'
        )

        self.ad = BulletinBoard.objects.create(
            title="RXS 590 8GB", price=7999,
            description="Good", author=self.user
        )

        self.review = Review.objects.create(
            text="Интересная видеокарта",
            author=self.user_1, ad=self.ad,
